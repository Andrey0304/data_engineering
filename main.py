...
def main():
    ...
    with conn.cursor() as cursor:
        ...
        users = create_data.create_users()
        users_id = users['id'].values.tolist()
        # cursor.execute('SELECT user_id FROM users;')
        # user_id_list = tuple(map(lambda x: x[0], cursor.fetchall()))
        ...
        dividends = create_data.create_dividends(users_id, financial_instrument)
        interests = create_data.create_interests(dividends.copy(), financial_instrument, users_id)
