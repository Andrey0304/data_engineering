...
def create_users()->pd.DataFrame:
    ...
    users['name'] = [random.choice(boy_names + girl_names) for _ in range(200)]
    users['id'] = list(range(1, 201))
    users['surname'] = [random.choice(surnames) for _ in range(200)]
    ...
    users['passport_no'] = [f"{''.join(random.sample(string.ascii_uppercase, 2))}{code}"
                            for code in random.sample(range(10_000, 99_999), 200)]
    users['phone'] = ["(+374){}-{}".format(str(number)[:2], str(number)[2:])
                      for number in random.sample(range(10_000_000, 99_999_999), 200)]
    users['registration'] = pd.date_range(start='2021-01-01', end='2021-12-31', periods=200)
...
def create_dividends(user_id_list: list,
                     fin_instrument: pd.DataFrame)->pd.DataFrame:
    ...
    dividends.user_id = random.sample(user_id_list, 30)
    
    stocks = fin_instrument[fin_instrument['asset_category'] == 'Stocks']
    random_index = [random.choice(stocks.index) for _ in range(30)]
    ...
    dividends.date = [(pd.to_datetime('2021-01-01') + pd.to_timedelta(random.randint(1,300), unit='d')).date()
                      for _ in range(30)]
    dividends.amount = random.sample(range(-100, 200), 30)
...
def create_interests(datafrm_for_copying: pd.DataFrame,
                     ...
                     user_id_list: tuple)->pd.DataFrame:
    
    interest = datafrm_for_copying
    interest.user_id = random.sample(user_id_list, 30)

    bonds = fin_instrument[fin_instrument['asset_category'] == 'Bonds']
    random_index = [random.choice(bonds.index) for _ in range(30)]
...
def create_withholding_tax(datafrm_for_copying:pd.DataFrame)->pd.DataFrame:
    ...
    witholding_tax.amount = random.sample(range(-100, 0), 30)
...
def create_change_in_dividend_accruals(datafrm_for_copying:pd.DataFrame)->pd.DataFrame:
    ...
    change_in_dividend_accruals['quantity'] = random.sample(range(100, 5000, 100), 30)
...
def create_trades(fin_instrument: pd.DataFrame,
    ...
    random_index = [random.choice(fin_instrument.index) for _ in range(100)]
    ...
    trades.user_id = [random.choice(user_id_list) for _ in range(100)]
    ...
    trades.quantity = random.sample(range(1000, 500_000, 1000), 100)
    trades.t_price = random.sample(np.arange(1.5, 100, 0.5).tolist(), 100)
    trades.comm_fee = random.sample(np.arange(-50, 0, 0.2).tolist(), 100)
    trades.code = [random.choice(code_list) for _ in range(100)]
